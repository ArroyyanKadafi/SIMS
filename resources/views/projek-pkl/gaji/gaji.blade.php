        ...
        <ul class="navbar-nav mr-lg-2">
          <li class="nav-item nav-search d-none d-lg-block">
            <div class="input-group">
              ...
              <!-- Search -->
              <form action="/penggajian/cari" method="GET">
          ...
          <div class="row">
            <!-- <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="row">
                  <div class="col-md-6"> -->
                    <h3 class="ti-credit-card mr-3 mt-3 "> List of Teacher Payroll</h3>
                    ...
                    <table class="table table-hover table-light text-center mt-3">
                      <thead class="bg-primary text-white">
                        <tr>
                          ...
                          <th scope="col">Salary Receipts</th>
                          <th scope="col">Pay Cut</th>
                      ...
                      <tbody>
                          ...
                          <td>{{ $dataGaji->nip }}</td>
                          <td>{{ $dataGaji->nama}}</td>
                          <td>{{ $dataGaji->bulan}}</td>
                          <td>{{ $dataGaji->tanggal}}</td>
                          <td>{{ $dataGaji->penerimaan}}</td>
                          <td>{{ $dataGaji->potongan}}</td>
                          <td>{{ $dataGaji->honor_perjam}}</td>
                          <td>{{ $dataGaji->total_jam}}</td>
                          <td>{{ $dataGaji->bayar}}</td>
                          <td>
                            <a href="{{ url('/penggajian/edit/'.$dataGaji->id) }}" class="btn btn-success ti-pencil-alt mr-2"> Edit</a> 

                              <form action="{{ url('/penggajian/delete/'.$dataGaji->id) }}" method="POST" class="d-inline">
                              @method('delete')
                              @csrf
                              <!-- <input type="hidden" name="_method" value="DELETE"> -->
                              <button type="submit" class="btn btn-danger ti-trash mr-2 border-0" onclick="return confirm('Are you sure ?')"> Delete</button>
                              
                              <a href="{{ url('/penggajian/print/' .$dataGaji->id) }}" target="_blank" class="btn btn-warning mt-2 ti-printer mr-2"> Print</a>
                            </form>  
                          </td>                          
                        </tr>                        
                        @endforeach
                    ...
                    <a href="{{ url('/penggajian/create') }}" class="btn btn-info mt-2 ti-plus mr-2 mb-5"> Add Payments</a> 
                  </div>
                  </div>
                  <div class="col-md-6">
                </div>
